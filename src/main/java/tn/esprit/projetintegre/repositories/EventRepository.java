...
public interface EventRepository extends JpaRepository<Event, Long> {
    ...
    @EntityGraph(attributePaths = { "site", "organizer" })
    @Query("SELECT e FROM Event e WHERE e.status = 'PUBLISHED' AND e.endDate > :now ORDER BY e.endDate")
    List<Event> findUpcomingEvents(LocalDateTime now, Pageable pageable);

    @EntityGraph(attributePaths = { "site", "organizer" })
    @Query("SELECT e FROM Event e WHERE e.status = 'PUBLISHED' AND (LOWER(e.description) LIKE LOWER(CONCAT('%', :keyword, '%')))")

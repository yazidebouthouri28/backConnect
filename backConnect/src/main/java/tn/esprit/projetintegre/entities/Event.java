...
public class Event {
    ...
    @OneToMany(mappedBy = "event", cascade = CascadeType.ALL, orphanRemoval = true)
    @Builder.Default
    private List<EventServiceEntity> requestedServices = new ArrayList<>();
    ...
    public Long fetchOrganizerUserId() {
        return (organizer != null && organizer.getUser() != null) ? organizer.getUser().getId() : null;
    }

    @JsonProperty("organizerId")
    public Long fetchOrganizerId() {

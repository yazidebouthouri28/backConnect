...
public class ParticipantController {
    ...
    public ResponseEntity<ApiResponse<List<ParticipantResponse>>> getByEventId(
            @PathVariable Long eventId,
            Pageable pageable) {
        return ResponseEntity.ok(ApiResponse.success(participantService.getByEventId(eventId, pageable).getContent()));
    }

    @GetMapping("/user/{userId}")
    @Operation(summary = "Récupère les participants par ID utilisateur")
    public ResponseEntity<ApiResponse<List<ParticipantResponse>>> getByUserId(@PathVariable Long userId) {
        return ResponseEntity.ok(ApiResponse.success(participantService.getByUserId(userId)));
    }
    ...
    @PostMapping
    @PreAuthorize("hasAnyRole('USER', 'ORGANIZER', 'ADMIN', 'PARTICIPANT')")
